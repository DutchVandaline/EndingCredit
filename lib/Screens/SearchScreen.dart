...
class _SearchScreenState extends State<SearchScreen> {
    ...
    await showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("정말 추가하시겠습니까?", style: TextStyle(color: Colors.white)),
          actions: <Widget>[
            InkWell(
              onTap: () {
                saveMovie(context);
  ...
  void saveMovie(BuildContext context) {
    DateTime now = DateTime.now();
    String id = memosha512(now.toString());
    String changedDate = DateFormat('yyyy년 MM월 dd일 hh시 mm분').format(now);
    Provider.of<MovieNotifier>(context, listen: false).addMovie(
      id,
      movietitle,
      '',
      changedDate,
    ...
    localid = id;
    localtitle = movietitle;
    localEditTime = changedDate;
